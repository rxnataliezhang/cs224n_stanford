...
def softmax(x):
    ...
    x_sub = x - x_max
    numer = tf.exp(x_sub)
    denom = tf.reduce_sum(numer, axis=1, keepdims=True)
    out = tf.divide(numer, denom)
...
def cross_entropy_loss(y, yhat):
    ...
    ### YOUR CODE HERE
    y = tf.to_float(y)
    out = tf.reduce_sum(tf.multiply(y, tf.log(yhat)), axis=1, keepdims=True)
...
if __name__ == "__main__":
    test_softmax_basic()
    test_cross_entropy_loss_basic()import numpy as np
import tensorflow as tf
from utils.general_utils import test_all_close
