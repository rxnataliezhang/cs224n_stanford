...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
      ...
      for i in range(window_size):
        sentence_pad.insert(0, start)
        sentence_pad.append(end)
      
      sentence_unfold = functools.reduce(lambda x, y: x + y, sentence_pad, [])
      
      word_size = len(start)
      for i in range(len(sentence)):
        windowed_data.append((sentence_unfold[word_size*i: word_size*(2*window_size+1+i)], labels[i]))
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(input_emb, shape=[-1, self.config.n_window_features*self.config.embed_size])                                                  
